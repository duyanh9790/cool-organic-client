...
const AboutUs = () => {
  return (
    <div className='container py-12 text-center about-us'>
      <h2 className='pb-5 text-4xl uppercase text-gradient'>Về Chúng Tôi</h2>
      <span className='inline-block icon-green'></span>
      <p className='mt-4 mb-10 text-[#8b8b99] text-sm max-w-[840px] w-full mx-auto'>
      ...
      <div className='about-us-product-list'>
        {listItems.map((item, index) => (
          <div key={index} className='flex flex-col items-center'>
            ...
            <p className='leading-6 text-textColor max-w-[280px] w-full'>
              {item.description}
            </p>
          </div>
        ))}
